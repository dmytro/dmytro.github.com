---
title: Projects
layout: default
github_project: dmytro.github.com
tags:
  - projects
  - blog
  - personal page
summary: |
  Personal software projects
description: |
  This page contains short description of several software projects I was working on, either as part of my job, or for fun.
---
{% assign short = true %}

{% if page.url == "/index.html" %}{% else %}<a href="/">Back</a>{% endif %}

{% include links.textile %}

p(narrow). This page contains short description of several software projects I was working on, either as part of my job, or for fun. For software developed as part of my job, there's short description, and probably a couple of screen-shots. If it's a hobby, then it's an open source, and there's also pointer to source code. Please feel free to use, to re-distribute, to critic and to contribute.

This page itself is a project hosted on Github. You can download full source of it or clone it using git. Look at the don load links and 'Clone me' labels at the page.

{% include toc.html %}

h2. Open Source projects

{% include content/github.textile %}

--- 

{% include content/perl.textile %}

---

h2. Other

{% include content/enterprise.textile %}

---

{% include content/ukrainian.textile %}

---
